...
if [ ! -z $DB_USER ]; then
  ...
  if [ $? != 0 ]; then
    ...
    echo "GRANT ALL ON DATABASE $DB_NAME TO $DB_USER;" | sudo su -l postgres -c psql
...
if [ ! -f $ASSYNC_SERVICE_FILE_FULLPATH ]; then
  echo "[Unit]
Description=Sophia Asset Sync Service
Wants=network-online.target
After=syslog.target postgresql.service network-online.target

[Service]
Type=oneshot
User=$SERVICE_USER
WorkingDirectory=`pwd`
ExecStart=`pwd`/venv/bin/python3 asset_sync.py
StandardOutput=syslog
StandardError=syslog
SyslogIdentifier=$ASSYNC_SERVICE_NAME
" > $ASSYNC_SERVICE_FILE_FULLPATH
fi

if [ ! -f $ASSYNC_TIMER_FILE_FULLPATH ]; then
  echo "[Unit]
Description=periodically synchronize assets with SOPHIA cloud
...
sudo systemctl enable $ASSYNC_SERVICE_FILE
sudo systemctl enable $ASSYNC_TIMER_FILE
sudo systemctl start $SERVICE_FILE
sudo systemctl start $ASSYNC_TIMER_FILE
sudo systemctl status $SERVICE_FILE
sudo systemctl status $ASSYNC_TIMER_FILE
