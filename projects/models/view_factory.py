# The functions in this file are copied from sqlalchemy_utils.views
# Due to timescale there are some modifications
# Good reference for usage of views in sqlalchemy: http://www.jeffwidman.com/blog/847/
...
from . import db # remove

class CreateView(DDLElement):
    def __init__(self, name, selectable, materialized=False, timescale=False):
        self.name = name
...
def compile_create_materialized_view(element, compiler, **kw):
    return 'CREATE {}VIEW IF NOT EXISTS {} {} AS {} {}'.format(
        ...
        'data_raw.'+element.name,
        'WITH (timescaledb.continuous)' if element.timescale else '',
        compiler.sql_compiler.process(element.selectable, literal_binds=True),
        'WITH NO DATA' if element.timescale else ''
    )
...
def create_table_from_selectable(
    ...
    table = sa.Table(name, metadata, schema='data_raw',*args)#, autoload_with=db.engine)
...
def create_materialized_view(
    ...
    :param timescale: Boolean if it is a timescale continuous Materialized View
    ...
    # FIXME: Use sa. or db.event listen ? 

    # FIXME: This is called after each Table creation -> Bad!! But if only listening for table -> it's not going to be called 
    sa.event.listen(metadata, 'after_create', CreateView(name, selectable, True, timescale))
    #sa.event.listen(metadata,'after_create', DDL("""SELECT add_continuous_aggregate_policy('data_raw.viewb6',
    #                                                  start_offset => NULL,
